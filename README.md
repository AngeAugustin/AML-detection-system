...
Packages live at the **repository root** (run commands from the project root).

```
aml_detection_system/
  __init__.py          # package marker (imports resolve from root)
data/
  synthetic_generator.py
  creditcard_adapter.py
  dataset_loader.py
  paysim_adapter.py    # PaySim CSV adapter (not wired into train_pipeline yet)
features/
...
utils/
  ...
  registry.py
  validation.py
artifacts/             # produced by training (models + schema.json)
requirements.txt
README.md
```
...
Optional body fields: `client_name`, `client_type`, `client_identifier` (defaults match `AnalyzeClientRequest` in `api/main.py`).
...
Réponse (`AnalyzeClientResponse`) : typologies et opérations mises en avant pour intégration métier ; les scores numériques restent internes au scoring.
...
```json
{
  ...
  "clientIdentifier": "C0001",
  "suspicionTypologies": ["UNUSUAL_BEHAVIOR"],
  "suspicionDescription": "Activités inhabituelles détectées sur une courte période.",
  "suspiciousOperations": [
    {
      "operationDate": "2026-03-01T10:11:12.000Z",
      "operationType": "TRANSFER",
      "amount": 1200.0,
      "currency": "EUR",
      "description": "Operation via online",
      "suspicionReason": "Comportement atypique"
    }
  ]
}
```

Les enregistrements renvoyés par `GET /findings` sont le même objet sérialisé (liste dans `items`, avec `count`).
...
- `isolation_forest.joblib`, `xgboost.joblib`, `autoencoder.joblib` : modèles sérialisés.
- `schema.json` : liste `feature_names` alignée sur l’inférence.
...
- **API** : validation Pydantic sur les requêtes (montant ≥ 0 et ≤ seuil, longueur des chaînes, nombre max de transactions par requête). Aucune désérialisation non sécurisée (pas de `pickle` chargé depuis l’extérieur). CORS : origine autorisée `http://localhost:5173` (voir `api/main.py`).
...
- Artifacts are versioned by filename and include model parameters + feature schema (`schema.json`).
- The `/analyze-client` endpoint is stateless and scores from the request payload. External ingestion keeps an in-memory rolling history per `client_id` for `/findings`.
- Explainability uses SHAP for the XGBoost model when available; otherwise falls back to feature importance + rule-based explanations (surfaced in the internal narrative used for `suspicionDescription` / typologies).
